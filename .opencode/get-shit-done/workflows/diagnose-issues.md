...
```bash
AGENT_SKILLS_DEBUGGER=$(node "D:/Projects/Godot/三国模拟器/.opencode/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-debugger 2>/dev/null)
```
...
```bash
node "D:/Projects/Godot/三国模拟器/.opencode/get-shit-done/bin/gsd-tools.cjs" commit "docs({phase_num}): add root causes from diagnosis" --files ".planning/phases/XX-name/{phase_num}-UAT.md"
```
</step>
