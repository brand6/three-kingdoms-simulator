...
```bash
node "D:/Projects/Godot/三国模拟器/.opencode/get-shit-done/bin/gsd-tools.cjs" validate health $REPAIR_FLAG
```
...
```bash
node "D:/Projects/Godot/三国模拟器/.opencode/get-shit-done/bin/gsd-tools.cjs" validate health
```
...
TASKS_DIR="D:/Projects/Godot/三国模拟器/.opencode/tasks"
if [ -d "$TASKS_DIR" ]; then
  STALE_COUNT=$( (find "$TASKS_DIR" -maxdepth 1 -type d -mtime +1 2>/dev/null || true) | wc -l )
  if [ "$STALE_COUNT" -gt 0 ]; then
    echo "⚠️  Found $STALE_COUNT stale task directories in D:/Projects/Godot/三国模拟器/.opencode/tasks/"
    echo "   These are leftover from crashed subagent sessions."
    echo "   Run: rm -rf D:/Projects/Godot/三国模拟器/.opencode/tasks/*  (safe — only affects dead sessions)"
  fi
fi
```
