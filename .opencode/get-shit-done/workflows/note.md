...
- **Global scope**: `D:/Projects/Godot/三国模拟器/.opencode/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/Projects/Godot/三国模拟器/.opencode/notes/`)
...
2. Glob `D:/Projects/Godot/三国模拟器/.opencode/notes/*.md` (if directory exists) — global notes
...
Global (D:/Projects/Godot/三国模拟器/.opencode/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/Projects/Godot/三国模拟器/.opencode/notes/` — works in any directory
