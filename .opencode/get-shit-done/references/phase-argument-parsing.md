...
```bash
PHASE_INFO=$(node "D:/Projects/Godot/三国模拟器/.opencode/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node "D:/Projects/Godot/三国模拟器/.opencode/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" --pick found)
...
```bash
PHASE_DIR=$(node "D:/Projects/Godot/三国模拟器/.opencode/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}" --raw)
```
